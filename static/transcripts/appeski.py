    ...
    try:
        ...
        with ThreadPoolExecutor(max_workers=3) as request_executor:
            ...
            for i, image in enumerate(images):
                ...
                prompt = (f"Match the product in this image with the name or code '{product_name_input}' and identify the product. "
                          f"Only write the product name clearly suitable for product search." if product_name_input else 
                          "Identify the product in this image and only write the product name clearly suitable for product search.")

                task = loop.run_in_executor(request_executor, partial(model.generate_content, [prompt, image_part[0]]))
                tasks.append(task)
        ...
        if len(product_names) == 1:
            combined_data_path, combined_data = await get_combined_product_data(product_names, username)
            if combined_data_path and combined_data:
            ...
            return jsonify({"error": "Invalid number of products."}), 400
